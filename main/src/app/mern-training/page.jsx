...
import { 
  ...
  Sparkles, 
  Tag,
  TicketPercent // New icon for the ticket inside form
} from 'lucide-react';
...
export default function MernTrainingPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {await fetch('https://rajabackend.srikrishnatechhub.com/request-callback', {
      ...
      // TODO: Replace with your actual fetch call
      // const res = await fetch('/api/enroll', { method: 'POST', body: JSON.stringify(dataToSend) ... })
  ...
  // Function to handle "Claim Offer" click from Pricing Card
        ...
        <div className="max-w-4xl mx-auto px-4 text-center">
          ...
          <form onSubmit={handleSubmit} className={`relative bg-slate-50 p-8 rounded-3xl shadow-sm border max-w-md mx-auto text-left transition-colors duration-500 ${isDiscountActive ? 'border-green-400 shadow-xl shadow-green-100' : 'border-slate-100'}`}>
            ...
            <div className="mb-4">
              ...
              <input required type="tel" name="phone" value={formData.phone} onChange={handleChange} placeholder="+91 98765 43210" className="w-full text-white text-slate-900 px-4 py-2 rounded-lg border border-slate-300 focus:ring-2 focus:ring-indigo-500 focus:outline-none" />
            </div>

            {/* --- THIS IS THE NEW VISUAL OFFER TICKET INSIDE THE FORM --- */}
            {isDiscountActive && (
      ...
      <footer className="bg-slate-900 text-slate-300 py-12">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex flex-col md:flex-row justify-between items-center">
          ...
          <div className="flex space-x-6 text-sm">
            ...
            <button onClick={() => enrollSectionRef.current?.scrollIntoView({ behavior: 'smooth' })} className="hover:text-white transition">Contact Us</button>
          </div>
        </div>
