...
import {
    ...
    ChevronDown, AlertCircle, X, ShieldCheck
} from 'lucide-react';
...
export default function CourseDetailPage() {
    ...
    // 1. Ensure params.id is treated as a string
    const paramIdString = Array.isArray(params.id) ? params.id[0] : params.id;

    // 2. Use parseInt. 
    // parseInt("1-mern-stack") returns 1. 
    // Number("1-mern-stack") returns NaN.
    const courseId = parseInt(paramIdString as string);
    ...
    useEffect(() => {
        const checkLoginStatus = async () => {
            ...
            try {
                ...
                if (response.ok) {
                    // API returned 200 OK - User is valid
                    setIsLoggedIn(true);
                } else {
    ...
    return (
        <div className="min-h-screen bg-slate-50 pb-20 font-sans selection:bg-indigo-100 selection:text-indigo-800 relative">
            ...
            {/* --- MODAL: LOGIN REQUIRED (If isLoggedIn === false) --- */}
            {showLoginModal && (
            ...
            {/* --- MODAL: CONFIRM PURCHASE (If isLoggedIn === true) --- */}
